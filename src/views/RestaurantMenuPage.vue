...
import { formatPrice, groupMealsByCategory, isRestaurantOpenNow } from '../utils/menu'
...
const fetchMenu = async () => {
  ...
  try {
    ...
    selectedCategory.value = Object.keys(grouped)[0] || ''
    await nextTick()
...
const updateActiveCategory = () => {
  if (!categoryEntries.value.length) {
    return
  }

  const firstCategory = categoryEntries.value[0][0]
  ...
  categoryEntries.value.forEach(([category]) => {
    const section = document.getElementById(`category-${category}`)
...
<template>
  <main class="relative min-h-screen overflow-hidden bg-[linear-gradient(135deg,#1a1008_0%,#231F20_50%,#0d0d0d_100%)] pb-16 pt-[70px] text-white">
    ...
    <template v-else-if="restaurant">
      ...
      <section class="sticky top-[70px] z-40 bg-brand-primary/30 backdrop-blur-xl">
        <div class="mx-auto flex max-w-[980px] items-center gap-2 px-5 h-16 md:px-8">
          <button
            type="button"
      ...
      <section class="relative mx-auto max-w-[980px] px-5 py-8 md:px-8">
        <div v-if="categoryEntries.length === 0" class="rounded-[8px] border border-white/10 bg-white/10 px-5 py-10 text-center shadow-[0_16px_40px_rgba(0,0,0,0.28)] backdrop-blur-xl">
        ...
        <div v-else class="space-y-8">
          <section v-for="[category, meals] in categoryEntries" :id="`category-${category}`" :key="category" class="scroll-mt-36">
